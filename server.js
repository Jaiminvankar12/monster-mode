// ============================================================================
// 🐲 MONSTER MODE — PRODUCTION SERVER (FULLY FIXED)
// ============================================================================
...
// ✅ FIX: Use native fetch (Node 18+) OR node-fetch@2
let fetch;
try {
...
const MONGO_URI = process.env.MONGO_URI ||
    "mongodb+srv://jaiminvankar520_db_user:XLVuwi5Atn2RSBE1@cluster0.iea9sdz.mongodb.net/monster_database?retryWrites=true&w=majority";
...
// ============================================================================
// 🤖 GEMINI AI INIT (SINGLE INSTANCE — v2 SDK)
// ============================================================================
...
// 🤖 JARVIS AI CHAT MEMORY (permanent)
...
const habitSchema = new mongoose.Schema({
    ...
    description: String, startDate: String, endDate: String, createdAt: String
});
const Habit = mongoose.model('Habit', habitSchema);

const habitLogSchema = new mongoose.Schema({
    id: String, userId: String, habitId: String, date: String, completed: Boolean
});
const HabitLog = mongoose.model('HabitLog', habitLogSchema);

const workoutSchema = new mongoose.Schema({
    id: String, userId: String, name: String, sets: Number, value: Number,
    unit: String, category: String, startDate: String, createdAt: String
});
const Workout = mongoose.model('Workout', workoutSchema);

const workoutLogSchema = new mongoose.Schema({
    id: String, userId: String, workoutId: String, date: String, completed: Boolean
});
const WorkoutLog = mongoose.model('WorkoutLog', workoutLogSchema);

const studyCategorySchema = new mongoose.Schema({
    id: String, userId: String, name: String, dailyTargetMinutes: Number,
...
const hygieneTaskSchema = new mongoose.Schema({
    id: String, userId: String, name: String, frequency: String, startDate: String
});
const HygieneTask = mongoose.model('HygieneTask', hygieneTaskSchema);

const hygieneLogSchema = new mongoose.Schema({
    id: String, userId: String, taskId: String, date: String, completed: Boolean
});
const HygieneLog = mongoose.model('HygieneLog', hygieneLogSchema);

const noteReminderSchema = new mongoose.Schema({
    id: String, userId: String, title: String, description: String,
    priority: { type: String, default: "Medium" }, dueTime: { type: String, default: "" },
    recurring: { type: String, default: "None" }, isReminder: Boolean, date: String,
    time: String, completed: Boolean, notifiedToday: Boolean, createdAt: String
});
const NoteReminder = mongoose.model('NoteReminder', noteReminderSchema);

const targetSchema = new mongoose.Schema({
    id: String, userId: String, name: String, date: String,
    completed: { type: Boolean, default: false }, createdAt: String
});
const Target = mongoose.model('Target', targetSchema);

const monsterLogSchema = new mongoose.Schema({
    id: String, userId: String, date: String, content: String,
    aiFeedback: String, createdAt: String
});
...
// ============================================================================
// 🔧 MIDDLEWARE SETUP (express.json FIRST, then session)
// ============================================================================
...
// ✅ FIX: express.json() and urlencoded() MUST come BEFORE routes
app.use(express.json({ limit: '5mb' }));
app.use(express.urlencoded({ extended: true, limit: '5mb' }));

// 🛡️ SYSTEM SLEEP GUARD
const checkSystemSleep = (req, res, next) => {
    if (systemSleepData.lockedUntil && Date.now() < systemSleepData.lockedUntil) {
        ...
        return res.status(423).json({
            success: false,
            error: `💤 SYSTEM IS ASLEEP: Locked for another ${remainingMins} minutes. (Even Admin is restricted).`
        });
    }
    next();
};
...
app.use(async (req, res, next) => {
    const trackerPages = ['/dashboard', '/dashboard.html', '/tracker', '/tracker.html',
        '/workout', '/workout.html', '/study', '/study.html', '/hygiene',
        '/hygiene.html', '/hydration', '/hydration.html'];
    const isTrackerPage = trackerPages.some(page => req.path === page);
    if (isTrackerPage && (!req.session || !req.session.userId)) {
...
// ✅ FIXED: moduleApiGuard now returns TRUE when allowed, FALSE when blocked
function moduleApiGuard(moduleName) {
    return async (req, res) => {
        let lockStatus = await getSystemLockStatus();
        if (lockStatus.locked) {
            res.status(403).json({ error: "🛡️ HARDCORE LOCK: System is totally locked." });
            return false;
        }
        if (moduleName === 'habits' && lockStatus.habitsLocked) { res.status(403).json({ error: "🔒 Habit Tracker is locked by Admin." }); return false; }
        if (moduleName === 'workouts' && lockStatus.workoutsLocked) { res.status(403).json({ error: "🔒 Workout Tracker is locked by Admin." }); return false; }
        if (moduleName === 'study' && lockStatus.studyLocked) { res.status(403).json({ error: "🔒 Study Tracker is locked by Admin." }); return false; }
        if (moduleName === 'hydration' && lockStatus.hydrationLocked) { res.status(403).json({ error: "🔒 Hydration Matrix is locked by Admin." }); return false; }
        if (moduleName === 'hygiene' && lockStatus.hygieneLocked) { res.status(403).json({ error: "🔒 Hygiene Tracker is locked by Admin." }); return false; }
        return true;
    };
}
...
// ✅ FIXED: TZ-safe hydration streak
async function calculateHydrationStreak(userId = MASTER_USER_ID) {
    ...
    while (true) {
        ...
        if (sanctuary.enabled && dateStr >= sanctuary.activatedAt) {
            streak++; d.setUTCDate(d.getUTCDate() - 1); continue;
        }
...
async function calculateHygieneStreak(userId = MASTER_USER_ID) {
    ...
    while (true) {
        ...
        let targetDateObj = new Date(dateStr + 'T00:00:00Z');
        let isSunday = targetDateObj.getUTCDay() === 0;
...
async function askGemini(promptText, fallbackText = "Execution is everything. Stop complaining.") {
    ...
    for (const modelName of modelsToTry) {
        try {
            ...
            const txt = (response.text || "").trim().replace(/^"|"$/g, '');
            if (txt) return txt;
        } catch (err) {
...
// ============================================================================
// 🤖 JARVIS AI — LIVE CONTEXT-AWARE ASSISTANT
// ============================================================================
async function buildJarvisContext(userId = MASTER_USER_ID) {
    const today = getServerToday();

    const [
        habits, workouts, hygieneTasks, studyCategories, targets,
        xpInfo, hydData, syncResult,
        workoutStreak, habitStreak, studyStreak, hygieneStreak, hydStreak
    ] = await Promise.all([
        Habit.find({ userId }),
        Workout.find({ userId }),
    ...
    return `
=== LIVE MONSTER MODE DASHBOARD DATA (${today}) ===
User Level: ${xpInfo.level} | XP: ${xpInfo.xp}
Streaks -> Habits: ${habitStreak}d | Workouts: ${workoutStreak}d | Study: ${studyStreak}d | Hygiene: ${hygieneStreak}d | Hydration: ${hydStreak}d

Today's Habits: ${habitsStatus}
Today's Workouts: ${workoutsStatus} | All done: ${syncResult.allWorkoutsDone ? 'YES' : 'NO'}
Today's Hygiene: ${hygieneStatus}
Today's Study: ${syncResult.totalStudiedMinutes}/${syncResult.totalTargetMinutes} min | Met: ${syncResult.studyDone ? 'YES' : 'NO'}
Today's Hydration: ${consumedWater}/${hydData.goal} ml | Met: ${syncResult.hydrationDone ? 'YES' : 'NO'}
Pending Milestones: ${pendingTargets.map(t => `${t.name} (due ${t.date})`).join('; ') || 'None'}
=== END LIVE DATA ===`.trim();
}

const JARVIS_SYSTEM_PROMPT = `You are "JARVIS", the personal AI assistant embedded inside the user's "Monster Mode" personal-discipline dashboard.

You have access to the user's LIVE dashboard data every message — ALWAYS use it for accurate, specific answers about their progress, streaks, and pending tasks.

You can also answer general knowledge, coding, fitness, study — anything — like a full general-purpose assistant.

Personality: Sharp, witty, loyal, slightly sarcastic (Tony Stark's JARVIS) — but genuinely useful.
Style: Concise, punchy. Light Markdown when helpful. No fluff.`;

async function callJarvisAI(userId, userMessage) {
    if (!process.env.GEMINI_API_KEY || process.env.GEMINI_API_KEY === "YOUR_GEMINI_API_KEY") {
        return "⚠️ Jarvis neural core offline — GEMINI_API_KEY not configured in .env";
    }
    ...
    for (const modelName of modelsToTry) {
        try {
            ...
            if (replyText) {
                console.log(`✅ Jarvis replied using: ${modelName}`);
                break;
            }
        } catch (err) {
...
app.post('/api/jarvis/chat', requireAuth, async (req, res) => {
    try {
        ...
        res.status(500).json({ error: "Jarvis is offline right now.", details: err.message });
    }
});

app.get('/api/jarvis/history', requireAuth, async (req, res) => {
    try {
        const messages = await JarvisMessage.find({ userId: MASTER_USER_ID })
...
app.post('/api/auth/login', async (req, res) => {
    ...
    if (!user || !bcrypt.compareSync(password, user.passwordHash)) {
        ...
        if (systemSleepData.failedAttempts >= 3) {
            systemSleepData.lockedUntil = Date.now() + 30 * 60 * 1000;
            return res.status(423).json({
                success: false,
                error: "🚨 3 FAILED ATTEMPTS DETECTED. SYSTEM ENTERING 30-MINUTE DEEP SLEEP LOCKDOWN."
            });
        }
        return res.status(401).json({ error: `Wrong Password! (${attemptsLeft} attempts remaining.)` });
    }
...
app.post('/api/control-panel/login', async (req, res) => {
    ...
    if (!user || !bcrypt.compareSync(password, user.passwordHash)) {
        ...
        return res.status(401).json({ error: `🔒 Access Denied! (${attemptsLeft} attempts remaining.)` });
    }
    ...
    const now = Date.now();
    if (req.session.pendingAuth && req.session.pendingAuth.email === user.email && req.session.pendingAuth.sentAt && (now - req.session.pendingAuth.sentAt < 10000)) {
        return res.json({ success: true, requireOtp: true, message: "Admin authorization code already sent." });
    }
...
app.get('/api/control-panel/session', requireAuth, (req, res) => {
    res.json({ authenticated: true, email: "jaiminvankar520@gmail.com" });
});

// ============================================================================
// 📊 EXAM MODE / SANCTUARY / HYDRATION / NOTES
// ============================================================================
...
// ✅ FIXED: moduleApiGuard return value handling
app.post('/api/hydration/drink', requireAuth, trackerApiGuard, async (req, res) => {
    const ok = await moduleApiGuard('hydration')(req, res);
    if (!ok) return;
...
app.get('/api/notes-reminders', requireAuth, trackerApiGuard, async (req, res) => {
    ...
    let items = await NoteReminder.find({
        ...
        $or: [{ date: queryDate }, { isReminder: false }]
    });
...
app.post('/api/habits', requireAuth, trackerApiGuard, async (req, res) => {
    ...
    const newHabit = new Habit({
        id: 'hab_' + Date.now().toString(), userId: MASTER_USER_ID, name,
...
app.post('/api/workouts', requireAuth, trackerApiGuard, async (req, res) => {
    ...
    const newWorkout = new Workout({
        id: 'w_' + Date.now().toString(), userId: MASTER_USER_ID, name,
        sets: sets || 3, value: value || reps || 10, unit: unit || 'reps',
...
app.post('/api/workouts/:id/toggle', requireAuth, trackerApiGuard, async (req, res) => {
    const ok = await moduleApiGuard('workouts')(req, res);
    if (!ok) return;
...
app.post('/api/study/categories', requireAuth, trackerApiGuard, async (req, res) => {
    ...
    const newCat = new StudyCategory({
        id: 's_' + Date.now().toString(), userId: MASTER_USER_ID, name: name.trim(),
...
app.post('/api/study/sessions', requireAuth, trackerApiGuard, async (req, res) => {
    const ok = await moduleApiGuard('study')(req, res);
    if (!ok) return;
...
app.get('/api/hygiene', requireAuth, trackerApiGuard, async (req, res) => {
    ...
    const targetDate = req.query.date || today;
    const dateStatus = validateDateAccess(targetDate);
    let targetDateObj = new Date(targetDate + 'T00:00:00Z');
    let isSunday = targetDateObj.getUTCDay() === 0;
...
app.post('/api/hygiene', requireAuth, trackerApiGuard, async (req, res) => {
    ...
    const newTask = new HygieneTask({
        id: 'h_' + Date.now().toString(), userId: MASTER_USER_ID, name,
...
app.post('/api/hygiene/:id/toggle', requireAuth, trackerApiGuard, async (req, res) => {
    const ok = await moduleApiGuard('hygiene')(req, res);
    if (!ok) return;
        ...
        if (update && update.message && update.message.text) {
            ...
            if (text === '/status') {
                ...
                const coachMessage = await askGemini(
                    "Give a brutal David Goggins style roast for someone slacking on their goals. Keep it short. 1-2 sentences max.",
                    "Discipline equals absolute freedom."
                );
...
app.get('/api/badges', requireAuth, async (req, res) => {
    ...
    const badges = allBadges.map(b => ({ ...b, unlocked: lvl >= b.levelRequired }));
...
app.listen(PORT, () => {
    ...
    console.log(`🤖 JARVIS AI: ${process.env.GEMINI_API_KEY && process.env.GEMINI_API_KEY !== "YOUR_GEMINI_API_KEY" ? "ONLINE ✅" : "OFFLINE ❌ (Set GEMINI_API_KEY in .env)"}`);
});